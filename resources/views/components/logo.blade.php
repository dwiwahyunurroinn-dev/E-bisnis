...
{{-- Logo EcoCraft: badge daun + serat kayu, minimalis modern --}}
<span {{ $attributes->merge(['class' => 'ec-logo']) }} style="display:inline-flex;align-items:center;gap:10px;">
    <svg width="{{ $size }}" height="{{ $size }}" viewBox="0 0 48 48" fill="none" aria-label="EcoCraft">
        <defs>
    ...
    @if ($wordmark)
        <span style="font-weight:800;font-size:{{ round($size*0.5) }}px;letter-spacing:-.5px;color:{{ $light ? '#fff' : 'var(--primary-deep)' }};line-height:1;">
            Eco<span style="color:{{ $light ? 'rgba(255,255,255,.85)' : 'var(--ink)' }};">Craft</span>
        </span>
    @endif
</span>
